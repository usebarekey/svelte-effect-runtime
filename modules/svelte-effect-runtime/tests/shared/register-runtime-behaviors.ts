...
import type { EffectPreprocessOptions } from "$/v3/preprocess.ts";
import {
  compileFixtureModule as compile_fixture_module,
  destroyComponent,
  flushEffects,
  installDom,
  mountComponent,
  SVELTE_CLIENT_MODULE_URL,
} from "$tests/shared/support.ts";
import type { VersionHarness } from "$tests/shared/versions.ts";

export function register_runtime_behaviors(harness: VersionHarness): void {
  const { label } = harness;
  const ClientRuntime = harness.client
    .ClientRuntime as typeof import("$/v3/client.ts")[
      "ClientRuntime"
    ];
  const run_component_effect = harness.client
    .run_component_effect as typeof import("$/v3/client.ts")[
      "run_component_effect"
    ];
  const runtimeModuleId = harness.runtimeModuleUrl;

  async function compileFixtureModule(
    files: Record<string, string>,
    entrypoint: string,
    options: EffectPreprocessOptions & { compileOptions?: unknown } = {},
  ) {
    return await compile_fixture_module(files, entrypoint, {
      compileOptions: options.compileOptions as never,
      preprocessor: harness.preprocess({
        effectModuleId: options.effectModuleId,
        runtimeModuleId: options.runtimeModuleId ?? runtimeModuleId,
  ...
  Deno.test(`[${label}] mounted effect components can assign into top-level Svelte state`, async () => {
...
`,
  ...
  Deno.test(`[${label}] later declarations can use yielded values like normal Effect code`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] component unmount cancels the running effect`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] effect components with no provided services mount via the default runtime`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] ClientRuntime.make auto-provides services to effect components`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] destructuring yielded values works in effect declarations`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] markup helpers support inline handlers and block expressions`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] markup helpers still work when Svelte experimental async mode is enabled`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] inline yield event handlers respect exact effect semantics from the component`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] failed effects surface as uncaught client errors`, async () => {
    ...
    try {
      const cleanup = run_component_effect(
        runtime,
  ...
  Deno.test(`[${label}] ClientRuntime composes through Layer.provide for runtime dependencies`, async () => {
  ...
  Deno.test(`[${label}] ClientRuntime unions multiple provided services into one runtime`, async () => {
    const NumberService = Context.GenericTag<{ readonly value: number }>(
      "test/NumberService",
    );
  ...
  Deno.test(`[${label}] {@render yield* fn(arg)} mounts the returned snippet`, async () => {
    const dom = installDom();
...
`,
  ...
  Deno.test(`[${label}] effect components mount without an explicit ClientRuntime.make() call`, async () => {
    const dom = installDom();
